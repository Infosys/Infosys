...
type Config struct {
	...
	FileStoreURL         string // File storage service URL
	RedisAddr            string // Redis server address
	OnboardingServiceURL string // Onboarding service URL
}
...
func Load() *Config {
	return &Config{
		...
		FileStoreURL:         getEnv("FILESTORE_URL", "http://localhost:8085/filestore/v1/files"),
		RedisAddr:            getEnv("REDIS_ADDR", "http://localhost:32380"),
		OnboardingServiceURL: getEnv("ONBOARDING_SERVICE_URL", "http://localhost:8085")}
}
