...
export const AgentCard: React.FC<AgentCardProps> = ({ agent, onClick }) => {
  ...
  const ONLINE_ICON_COLOR = isAvailable ? "#00703C" : "#A80000";
    ...
    >
      ...
      >
        <Box sx={{ display: "flex", alignItems: "flex-start", gap: 1.2 }}>
          <AccountCircleOutlinedIcon sx={{ fontSize: 32, color: "#222" }} />
          <Box>
            {/* Agent's full name */}
            <Typography
              sx={{
            ...
            >
              <b>Wards:</b>{" "}
              {getWardsDisplay(agent.zoneData?.flatMap((z) => z.wards))}
            </Typography>
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
          ...
          {isAvailable ? (
            <CheckCircleOutlineIcon
              sx={{ fontSize: 12, color: ONLINE_ICON_COLOR, mb: 0.3 }}
            />
          ) : (
            <CancelOutlinedIcon
              sx={{ fontSize: 12, color: OFFLINE_TXT_COLOR, mb: 0.3 }}
            />
          )}
        </Box>
      </Box>
