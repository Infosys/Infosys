...
const ALLOWED_ROLES = ["SERVICE_MANAGER", "COMMISSIONER", "ADMIN"] as const;
...
class AuthService {
  ...
  public primaryRole: "SERVICE_MANAGER" | "COMMISSIONER" | "ADMIN" | null =
    null;
  ...
  async loginWithPassword(credentials: AuthCredentials): Promise<boolean> {
    try {
      ...
      // Temporarily store tokens (not in localStorage yet)
      const tempAccessToken = tokenData.access_token;
      const tempRefreshToken = tokenData.refresh_token;
      const tempTokenExpiry = Date.now() + tokenData.expires_in * 1000;

      // Decode token immediately to check role BEFORE storing
      this.accessToken = tempAccessToken;
      const tokenRole = this.getPrimaryRoleFromToken();

      // Fetch user profile to determine role
      let user: User | null = null;
      let primaryRole: "SERVICE_MANAGER" | "COMMISSIONER" | "ADMIN" | null =
        null;
      ...
      // SECURITY CHECK: Reject unauthorized roles (AGENT, CITIZEN, etc.)
      if (!primaryRole || !ALLOWED_ROLES.includes(primaryRole)) {
        console.warn(
          `Login blocked for unauthorized role: ${primaryRole || "UNKNOWN"}`
        );
      ...
      // Role is authorized - NOW store tokens in localStorage
      this.accessToken = tempAccessToken;
      this.refreshToken = tempRefreshToken;
      this.tokenExpiry = tempTokenExpiry;

      localStorage.setItem("access_token", this.accessToken);
      localStorage.setItem("refresh_token", this.refreshToken);
      localStorage.setItem("token_expiry", this.tokenExpiry.toString());

      // Set role-specific locale key
      localeKey =
  ...
  private loadFromStorage(): void {
    ...
    const storedServiceManagerLocale = localStorage.getItem(
      "serviceManagerLocale"
    );
  ...
  async logout(): Promise<void> {
    try {
      if (this.user) {
        const lastLocale =
          localStorage.getItem(this.localeKey) || getAppLocale() || "en";
        console.log(
          "Calling updatePreferredLanguage with:",
          this.user.id,
          lastLocale
        );
        try {
  ...
  getPrimaryRole(): "SERVICE_MANAGER" | "COMMISSIONER" | "ADMIN" | null {
    if (this.user?.role) {
      if (this.user.role.toUpperCase() === "SERVICE_MANAGER")
        return "SERVICE_MANAGER";
      if (this.user.role.toUpperCase() === "COMMISSIONER")
        return "COMMISSIONER";
